...
/*My own custom writer function to make sure the output does not get dumped to console only
it is extern "C", as it's a c function but being used in CPP to make sure we don't get any typo*/
...
class CurlHandle {
  private :
    ...
    //Defining the custom deleter for memory cleanup
    //easy_cleanup ensures CURL* c void pointer cleanup
    //easy_gloabal claims the memory back allocated by curl_global_init
    //We should call curl_global_cleanup once for each call we make to curl_global_init, after you are done using libcurl.
      curl_easy_cleanup(c);
      curl_global_cleanup();
    };
  ...
  public :
    ...
    //Fetch from fetch() functions and dump it into the string datatype
